...
if __name__ == "__main__":
    ...
    with st.sidebar:
        ...
        st.caption("📄 arXiv 论文抓取")
        crawl_since = st.text_input("起始日期 (YYYYMMDD)，留空为增量", placeholder="如 20260201", key="crawl_since")
        if st.button("触发抓取", use_container_width=True, key="btn_crawl"):
            with st.spinner("正在启动抓取任务..."):
                result = api.trigger_crawl(since=crawl_since.strip() or None)
            if result and result.get("code") == 200:
                st.toast(result.get("msg", "任务已启动"))
            else:
                st.error(result.get("msg", "启动失败") if result else "API 无响应")
