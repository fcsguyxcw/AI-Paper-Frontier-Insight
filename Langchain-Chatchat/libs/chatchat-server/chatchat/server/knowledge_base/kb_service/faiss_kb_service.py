...
class FaissKBService(KBService):
    ...
    def do_search(
        ...
        with self.load_vector_store().acquire() as vs:
            if filter:
                filter_fn = self._build_filter_fn(filter)
    ...
    @staticmethod
    def _build_filter_fn(filter: Dict) -> Callable:
        """将 filter dict 转为 FAISS 可用的过滤函数

        支持格式:
        - {"published": "2026-01-15"}                        # 精确匹配
        - {"published": {"$gte": "2026-01-01"}}               # 大于等于
        - {"published": {"$gte": "2026-01-01", "$lte": "2026-03-01"}}  # 范围
        """
