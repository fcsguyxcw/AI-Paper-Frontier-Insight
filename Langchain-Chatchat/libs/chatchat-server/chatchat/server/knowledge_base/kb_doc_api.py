...
def search_docs(
    ...
    if kb is not None:
        if query:
            docs = kb.search_docs(query, top_k, score_threshold, filter=metadata)
            data = []
            for x in docs:
                if isinstance(x, tuple):
                    # faiss_kb_service 带 filter 时返回 (Document, float)
                    doc = DocumentWithVSId(**x[0].dict(), id=x[0].metadata.get("id"))
                else:
                    doc = DocumentWithVSId(**{"id": x.metadata.get("id"), **x.dict()})
                data.append(doc)
