...
async def trigger_crawl(since: str = Body(None)):
    ...
    def _run():
        try:
            from chatchat.crawlers.pipeline import ArxivPipeline
            ArxivPipeline().run(since=since)
        except Exception as e:
            _logger.error(f"论文抓取失败: {e}")

    loop = asyncio.get_running_loop()
    loop.run_in_executor(None, _run)
    return BaseResponse(code=200, msg=f"抓取任务已启动 (since={since or '增量模式'})，请查看服务器日志了解进度。")
