...
def get_httpx_client(
    ...
    """
    # 不代理本地地址（用 scheme-specific 格式，httpx 0.27+ 中 all:// 不可靠）
    default_proxies = {}
    for scheme in ("http://", "https://"):
        default_proxies.update({
            scheme + "127.0.0.1": None,
            scheme + "localhost": None,
        })
        # do not use proxy for user deployed fastchat servers
        for x in unused_proxies:
            host = ":".join(x.split(":")[:2])
            default_proxies.update({scheme + host: None})
