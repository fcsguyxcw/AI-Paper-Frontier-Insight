...
class KBSettings(BaseFileSettings):
    ...
    # ----- arXiv 论文爬虫配置 -----
    ARXIV_CATEGORIES: t.List[str] = ["cs.AI", "cs.CL", "cs.CV", "cs.IR", "cs.LG"]
    """arXiv 抓取分类列表"""

    ARXIV_MAX_RESULTS_PER_CATEGORY: int = 1000
    """每个分类单次最大抓取结果数"""

    ARXIV_CRAWL_INTERVAL: float = 3.0
    """arXiv API 请求间隔（秒）"""

    ARXIV_PAPER_KB_NAME: str = "arxiv_papers"
    """论文知识库名称"""

    USE_RERANKER: bool = False
    """是否启用 Cross-Encoder 重排序（需安装 sentence-transformers 并下载模型）"""
